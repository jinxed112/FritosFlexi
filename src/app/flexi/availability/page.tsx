...
function getAvailStyle(state: DayState) {
  if (!state) return { bg: 'bg-gray-100', text: 'text-gray-400', dot: '' };
  switch (state.type) {
    case 'available':
      return { bg: 'bg-emerald-400', text: 'text-white', dot: 'bg-emerald-400' };
    case 'flexible':
      return { bg: 'bg-amber-300', text: 'text-white', dot: 'bg-amber-300' };
    case 'unavailable':
      return { bg: 'bg-red-300', text: 'text-white', dot: 'bg-red-300' };
  }
}
...
export default function FlexiAvailabilityPage() {
  ...
  // availMap: key = 'YYYY-MM-DD', value = DayState
  ...
  const dragDates = useRef<Set<string>>(new Set());
  ...
  const loadData = useCallback(async () => {
    ...
    (avails || []).forEach((a: any) => {
      map[a.date] = {
        ...
        preferred_location_id: a.preferred_location_id ?? null,
      };
    });
    setAvailMap(map);
  ...
  const openSheet = (dates: string[]) => {
    ...
    // Pre-fill with existing state of first selected date
  ...
  const handleDayTap = (dateStr: string) => {
    if (shifts[dateStr] || isPast(dateStr)) return;
    openSheet([dateStr]);
  };
  ...
  const handleDragStart = (dateStr: string) => {
    if (shifts[dateStr] || isPast(dateStr)) return;
    setIsDragging(true);
    dragDates.current = new Set([dateStr]);
  };

  const handleDragEnter = (dateStr: string) => {
    if (!isDragging || shifts[dateStr] || isPast(dateStr)) return;
    dragDates.current.add(dateStr);
  };

  const handleDragEnd = () => {
    if (!isDragging) return;
    setIsDragging(false);
    const dates = Array.from(dragDates.current);
    if (dates.length > 1) {
      openSheet(dates);
    }
    dragDates.current = new Set();
  };
  ...
  const counts = Object.values(availMap).reduce(
    (acc, s) => {
      if (!s) return acc;
  ...
  return (
    <div className="pb-4">
      ...
      <div className="flex items-center justify-between mb-4">
        <button
          onClick={prevMonth}
          className="w-9 h-9 rounded-xl bg-gray-100 flex items-center justify-center text-gray-500 hover:bg-gray-200 active:scale-95 transition-all"
        >
          ←
        </button>
      ...
      {/* Calendar */}
      <div
        className="bg-white rounded-2xl border border-gray-100 shadow-sm p-3 select-none"
          ...
          {cells.map((day, i) => {
            ...
            return (
              <button
                ...
                onTouchStart={() => handleDragStart(dateStr)}
                onTouchMove={(e) => {
                  const touch = e.touches[0];
                  const el = document.elementFromPoint(touch.clientX, touch.clientY);
                  const d = el?.getAttribute('data-date');
                  if (d) handleDragEnter(d);
                }}
                data-date={dateStr}
      ...
      <p className="text-[11px] text-gray-400 text-center mt-3">
        Tapez un jour ou glissez sur plusieurs jours
      </p>

      {isPending && (
        <p className="text-[11px] text-orange-500 text-center mt-1">Sauvegarde...</p>
      )}
      ...
      {sheetOpen && (
        <>
          ...
          <div className="fixed bottom-0 left-0 right-0 z-50 bg-white rounded-t-3xl shadow-2xl max-w-md mx-auto px-5 pt-4 pb-8">
              ...
              {[
                {
                  ...
                  emoji: '🟢',
                  label: 'Disponible',
                  desc: 'Je veux travailler ce jour',
                  active: 'border-emerald-400 bg-emerald-50',
                  inactive: 'border-gray-100',
                },
                {
                  type: 'flexible' as AvailabilityType,
                  emoji: '🟡',
                  label: 'Flexible',
                  desc: 'Je peux me libérer si besoin',
            ...
            {/* Site preference (only if available or flexible) */}
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 py-3 rounded-2xl bg-gray-100 text-gray-500 font-medium text-sm hover:bg-gray-200 transition-colors"
              >
                Effacer
              </button>
              <button
                ...
                disabled={!sheetType}
                className="flex-2 flex-grow-[2] py-3 rounded-2xl bg-orange-500 text-white font-semibold text-sm hover:bg-orange-600 transition-colors disabled:opacity-50"
              >
                Confirmer
              </button>
            </div>
