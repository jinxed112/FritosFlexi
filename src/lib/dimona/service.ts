...
async function postDeclaration(payload: DimonaInPayload | DimonaCancelPayload | DimonaUpdatePayload): Promise<number> {
  ...
  if (response.status === 201) {
    ...
    try {
      ...
      if (body?.declarationStatus?.declarationId) return body.declarationStatus.declarationId;
    } catch { /* ignore */ }
...
async function pollDeclarationResult(
  ...
  while (Date.now() - startTime < maxPollTimeMs) {
    ...
    if (response.status === 200) {
      ...
      if (data.declarationStatus?.result) return data;
    }
    if (response.status === 404) { await sleep(pollIntervalMs); continue; }
    const text = await response.text();
  ...
  try {
    const payload: DimonaInPayload = {
      ...
      dimonaIn: workerType === 'STU' ? {
        ...
        features: { workerType, jointCommissionNumber: 'XXX' },  // STU Horeca: 'XXX' aussi (convention ONSS — vérifié sur 192 déclarations STU OK avril-mai 2026 avec cette valeur)
      } : {
        startDate: date,
...
// --- Utilities ---
function cleanNiss(niss: string): string { return niss.replace(/[\.\-\s]/g, ''); }
function formatHour(time: string): string { return time.replace(/[:.]/g, '').slice(0, 4); }
